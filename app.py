...
#   _auto_symbol_refresh_loop(): arka planda çalışır, her 7 günde bir
#   symbols_builder.build_top_usdt() çağırarak sembol listesini günceller
#   Bu sayede TRUMPUSDT/NIGHTUSDT gibi düşüşteki coinler otomatik elenır,
#   momentum kazanan yeni coinler listeye girer
import json
import time
import threading
import subprocess
...
_wf_lock       = threading.Lock()
_wf_active_dir = ""
_last_symbol_refresh = 0.0   # son sembol güncelleme unix timestamp


# ── Haftalık Sembol Rotasyonu ─────────────────────────────────
def _auto_symbol_refresh_loop():
    """
    ÖNERİ 4: Her 7 günde bir symbols_builder.build_top_usdt() çağırır.
    Sistem çalışırken arka planda daemon thread olarak çalışır.
    Manuel güncelleme (update_symbols) bu sayacı sıfırlar.
    """
    global _last_symbol_refresh
    REFRESH_INTERVAL = 7 * 24 * 3600   # 7 gün saniye cinsinden

    # İlk çalışmada kısa bekle (sistem tam başlasın)
    time.sleep(30)
    ...
    while True:
        ...
        time.sleep(3600)   # her saat kontrol et, güncelleme zamanı geldiyse yap


# Rotasyon thread'ini başlat
...
def _make_bt_dir(params: dict) -> str:
    ...
    try:
        ...
        snap  = {
            ...
            # Adaptive Symbol Quality Filter
...
class API:
    ...
    def update_symbols(self):
        def _run():
            ...
            try:
                from symbols_builder import build_top_usdt
                build_top_usdt()
                _last_symbol_refresh = time.time()   # ÖNERİ 4: sayacı sıfırla
    ...
    def get_wf_history(self):
        if not os.path.exists(WF_BASE_DIR):
            return []
        folders = sorted([d for d in os.listdir(WF_BASE_DIR)
    ...
    def get_wf_results(self, folder: str):
        ...
        try:
            with open(os.path.join(path,"wf_monthly.csv"),
                ...
                out["monthly"] = list(csv.DictReader(f, delimiter=";"))
        except Exception:
            pass
        return out
